...
func (s *CategoryDeletionWorkerTestSuite) TestDoesntNullCategoryIdsIfCategoryIsntDeleted() {
	transactionId := "some-cool-id"
	accountId := "acct-1234"
	categoryId := "some-cat-id"
	...
	err = s.categoryStore.Insert(data.Category{
		Id:        categoryId,
		AccountId: accountId,
	})
	...
	s.NoError(err)
	s.NotNil(transaction)
	s.NotNil(transaction.CategoryId)
}
